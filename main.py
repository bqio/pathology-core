...
IMAGE_PATH = "C:\\Users\\admin\\Desktop\\eye_pat.jpg"
...
def main():
    ...
    # Определяем рамку с отступом в 25 пикселей
    ...
    for i in range(image.height):
        for j in range(image.width):
            ...
            if pixel <= RGBA_LIMIT and pixel != RGBA_TRANSPARENT:
                image.putpixel((i, j), RGBA_RED)
                black_pixel_count += 1

    # Высчитываем процент пикселей с патологией от общего количества пикселей
    total_pixels = len(pixels) - transparent_pixel_count
